...
# async def admin_types_kb(back=True):
#     builder = InlineKeyboardBuilder()
#     types = await rq.get_types()
#     for type in types:
#         button = InlineKeyboardButton(text=type.ru_name, callback_data=f"admin_type_{type.id}")
#         builder.row(button)
#     if back:
#         builder.row(InlineKeyboardButton(text="⏪Вернуться", callback_data="back_menu"))
#     return builder.as_markup()
...
#         [KeyboardButton(text="✏️ Редактировать товар"), KeyboardButton(text="📊 Статистика")]
#     ],
#     resize_keyboard=True
# )
